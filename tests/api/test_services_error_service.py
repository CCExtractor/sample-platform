...
class TestServicesErrorService(ApiTestCase):
    def setUp(self):
        super().setUp()
    ...
    def test_derive_errors_for_run_rc_mismatch(self):
        ...
        # The expected output was produced and matched (got=None), so the
        # only error left is the exit-code mismatch.
    ...
    def test_derive_errors_for_run_rc_mismatch_without_files_adds_missing(self):
        # No result files at all: the rc mismatch is reported AND the
        # expected output counts as missing — matching /runs/{id}/summary.
        tr = TestResult(self.test_obj.id, self.reg_test1.id, 100, 1, 0)
        g.db.add(tr)
        g.db.commit()

        errors = derive_errors_for_run(self.test_obj.id)
        types = sorted(e['type'] for e in errors)
        self.assertEqual(types, ['exit_code_mismatch', 'missing_output'])
    ...
    def test_derive_error_summary(self):
        tr1 = TestResult(self.test_obj.id, self.reg_test1.id,
                         100, 1, 0)  # rc mismatch
        # Matched output for reg_test1 so tr1 contributes only the rc error.
    ...
    def test_derive_infrastructure_errors_excludes_api_cancels(self):
        # A user/API cancellation must not be reported as an infrastructure error.
        tp_api = TestProgress(
            self.test_obj.id, TestStatus.canceled, 'Canceled by alice via API')
        tp_api.timestamp = datetime.datetime(2023, 1, 1, 10, 0, 0)
        tp_infra = TestProgress(
            self.test_obj.id, TestStatus.canceled, 'build failed')
        tp_infra.timestamp = datetime.datetime(2023, 1, 1, 10, 5, 0)
        g.db.add(tp_api)
        g.db.add(tp_infra)
        g.db.commit()
        ...
        errors = derive_infrastructure_errors(self.test_obj.id)
        self.assertEqual(len(errors), 1)
        self.assertEqual(errors[0]['type'], 'build')
