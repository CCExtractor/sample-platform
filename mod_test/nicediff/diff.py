...
def eq(a, b, same_regions=None, delta_a=0, delta_b=0, only_onetype=False):
	...
	if same_regions is not None and index[zip(a)][zip(b)][0] != 0:
		...
		eq(sub_a_beg, sub_b_beg, same_regions=same_regions, delta_a=delta_a, delta_b=delta_b)
...
def get_html_diff(test_correct_lines, test_res_lines):
	...
	for line in range(min(len(test_res_lines), len(test_correct_lines))):
		html += '<table>'
		a, b = _process(test_res_lines[line], test_correct_lines[line], suffix_id=str(line))
		html += '<tr><td class="diff-table-td" style="width: 30px;">{line_id}</td><td class="diff-table-td">'.format(line_id=line + 1) + a + '</td></tr>'
		html += '<tr><td class="diff-table-td" style="width: 30px;"></td><td class="diff-table-td">' + b + '</td></tr>'
		html += '</table>'

	html += '<script>' + open('html/same-blocks-highlight.js').read() + '</script>'
	return html
