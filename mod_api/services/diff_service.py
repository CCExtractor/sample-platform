...
def read_lines(file_path: str, max_size_bytes: int = 10 * 1024 * 1024) -> List[str]:
    ...
    max_size_bytes : int
        Maximum file size in bytes. Raises ValueError if exceeded.
    """
    ...
    if file_size > max_size_bytes:
        raise ValueError(
            f"File too large ({file_size} bytes > {max_size_bytes} limit)")
    try:
        with open(file_path, encoding='utf8') as f:
            return [line.rstrip('\n\r') for line in f.readlines()]
    except UnicodeDecodeError:
        # errors='replace' because cp1252 leaves five byte values undefined —
        # without it the fallback can itself raise on binary output files.
