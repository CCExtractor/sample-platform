...
from tests.base import BaseTestCase, SignUpInformation
...
class TestSignUp(BaseTestCase):
    ...
    def test_existing_email_signup(self):
        response = self.signup(email=SignUpInformation.existing_user_email)
        self.assertEqual(response.status_code, 200)
    ...
    def test_valid_email_signup(self):
        response = self.signup(email=SignUpInformation.valid_email)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'Email sent for verification purposes. Please check your mailbox', response.data)
...
class CompleteSignUp(BaseTestCase):
    ...
    def setUp(self):
        ...
        content_to_hash = "{email}|{expiry}".format(email=SignUpInformation.valid_email, expiry=self.time_of_hash)
        self.hash = generate_hmac_hash(self.app.config.get('HMAC_KEY', ''), content_to_hash)
        content_to_hash = "{email}|{expiry}".format(email=SignUpInformation.existing_user_email,
    ...
    def test_if_link_expired(self):
        response = self.complete_signup(SignUpInformation.valid_email, self.time_of_hash+3600, self.hash)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'The request to complete the registration was invalid.', response.data)

    def test_if_wrong_link(self):
        response = self.complete_signup(SignUpInformation.existing_user_email, self.time_of_hash, self.hash)
        self.assertEqual(response.status_code, 200)
        self.assertIn(b'The request to complete the registration was invalid.', response.data)

    def test_if_valid_link(self):
        response = self.complete_signup(SignUpInformation.valid_email, self.time_of_hash, self.hash)
        self.assertEqual(response.status_code, 200)
        self.assert_template_used('auth/complete_signup.html')

    def test_if_password_is_blank(self):
        response = self.complete_signup(SignUpInformation.valid_email, self.time_of_hash, self.hash,
                                        name=SignUpInformation.existing_user_name, password='', password_repeat='')
        self.assertEqual(response.status_code, 200)
        self.assert_template_used('auth/complete_signup.html')
    ...
    def test_if_password_length_is_invalid(self):
        response = self.complete_signup(SignUpInformation.valid_email, self.time_of_hash,self.hash,
                                        name=SignUpInformation.existing_user_name, password='small',
    ...
    def test_if_passwords_dont_match(self):
        response = self.complete_signup(SignUpInformation.valid_email, self.time_of_hash, self.hash,
                                        name=SignUpInformation.existing_user_name, password='some_password',
