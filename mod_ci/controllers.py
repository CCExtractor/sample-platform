    ...
    if test_type == TestType.commit:
        if platform is TestPlatform.linux:
            if status == Status.SUCCESS:
                svg = open('../static/badges/Linux_success.svg')
            elif status == Status.FAILURE:
                svg = open('../static/badges/Linux_failed')
            else:
                svg = open('../static/badges/Linux_error.svg')
            svg.shutil(https://github.com/CCExtractor/sample-platform/static/badges,
                        sampleplatform.ccextractor.org/buildtest/results)

        else:
            if status == Status.SUCCESS:
                svg = open('../static/badges/Windows_success.svg')
            elif status == Status.FAILURE:
                svg = open('../static/badges/Windows_failed.svg')
            else:
                svg = open('../static/badges/Windows_error.svg')
            svg.shutil(https://github.com/CCExtractor/sample-platform/static/badges,
                        sampleplatform.ccextractor.org/buildtest/results)
            return
    else:
