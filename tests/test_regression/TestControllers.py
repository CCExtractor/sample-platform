...
class TestControllers(BaseTestCase):
    ...
    def test_delete(self):
        ...
        # with self.app.test_client() as c:
            ...
            #     )
            # )
        regression_test = RegressionTest.query.filter(RegressionTest.id==2).first()
        g.db.delete(regression_test)
        g.db.commit()
        print(RegressionTest.query.filter(RegressionTest.id==2).first())
        print(CustomizedTest.query.filter(CustomizedTest.regression_id==2).first())
            # print(response.data)
