...
class TestControllers(BaseTestCase):
    ...
    def test_regression_test_delete(self):
        ...
        with self.app.test_client() as c:
            response = c.get('/regression/test/15/delete')
            self.assertEqual(response.status_code, 404) # test whether we are getting an error for deleting an invalid regression test
            regression_test = RegressionTest.query.filter(RegressionTest.id == 1).first()
            self.assertNotEqual(regression_test, None)
            response = c.get('/regression/test/1/delete') # checks whether our function is Abel to delete present regression test
            regression_test = RegressionTest.query.filter(RegressionTest.id == 1).first()
            self.assertEqual(regression_test, None)                
